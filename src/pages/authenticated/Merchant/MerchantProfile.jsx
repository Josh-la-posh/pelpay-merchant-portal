...
function MerchantProfile() {
  ...
  const merchantService = useMemo(
    () => new MerchantService(axiosPrivate),
    [axiosPrivate]
  );
  const userService = useMemo(
    () => new UserService(axiosPrivate),
    [axiosPrivate]
  );
  ...
  const { merchantProfile, merchantProfileLoading, merchantProfileError } =
  ...
  useEffect(() => {
    ...
    setFormData((prev) => ({
      ...prev,
      userId:
        Array.isArray(aggregatorUser) && aggregatorUser.length
          ? aggregatorUser[0]?.id
          : "",
    }));
  }, [aggregatorUser]);
  ...
  useEffect(() => {
    ...
    //   console.log("Get the user role assigned permission", UserRolePermission)
      const assignedPermission = UserRolePermission.find((item) => {
      const name = item?.permission?.permissionName?.trim()?.toLowerCase();
      return name === "merchantprofile";
    });

    //   console.log("Get the merchant profile assigned permission", assignedPermission)
      if (assignedPermission) {
        setPermissions({
          canAdd: assignedPermission.canAdd,
          canEdit:  assignedPermission.canEdit,
          canDelete: assignedPermission.canDelete,
          canRead: assignedPermission.canRead,
        });
      }
    } catch (err) {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    await userService.fetchUserByAggregatorCode(
      aggregatorCode,
      1,
      20,
      dispatch
  ...
  const addMerchant = async () => {
    await merchantService.addUserMerchant(formData, dispatch);
  };
      ...
      <div className="space-y-5 sm:space-y-0 mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center">
        ...
        {isExpanded && (
          <div className="flex items-center gap-2 sm:gap-4">
                  ...
                  {canAddUser ? (
                    <div className="flex flex-row items-center justify-center gap-2">
                      <select
                        ...
                        className="flex-grow text-xs px-4 py-2 border border-gray-300 rounded-md outline-gray-400"
                      >
            ...
            {/* <Link
              to={`/merchants/profile/update/${merchantCode}`}
              className="bg-priColor text-xs text-white py-2 px-5 rounded-md text-center"
            >
